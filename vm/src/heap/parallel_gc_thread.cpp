...
#define DO_SIMPLE_CHECKPOINT_CHECK     1
...
void GC_Thread_Class::do_work()
{
  ...
  if(DO_NMT_SETTING_CHECK) {
    bool nmt = m_nilObj.getNMT();
    
    assert_always(m_nilObj.is_mem());
    Object* nil1 = m_nilObj.as_object();
    m_nilObj.setNMT(!m_nilObj.getNMT());
    assert_always(m_nilObj.is_mem() && m_nilObj.getNMT() == !nmt);
    Object* nil2 = m_nilObj.as_object();
...
void GC_Thread_Class::phase_mark(){
  ...
  printf("Finished mark phase (did %d mark_traverses)(%d %d %d)...\n",markCount,c1,c2,c3);
...
void GC_Thread_Class::mark_traverse(Object* object){
  ...
  if( ! m_pageLiveness[page].isAllocated() ) { return; }
  ...
  if (object->contains_class_and_type_word()) {
    ...
    mark_stack_.push( (Object*)class_oop.bits()); // Do conversion from oop to Object* yourself to not trigger read barrier.
...
void GC_Thread_Class::relocateLiveObjectsOfPage(Page* page){
  ...
  FOR_EACH_OBJECT_IN_UNPROTECTED_PAGE(unprotectedPage, object){
    if( !object->isFreeObject() && object->is_marked() ){
      object->relocateIfNoForwardingPointer( The_Memory_System()->heaps[Logical_Core::my_rank()] );
...
void GC_Thread_Class::remapAndUnMarkAllFromMarkStack(){
  while( !mark_stack_.is_empty()){
    ...
    if( object->is_marked()){
      ...
      FOR_EACH_STRONG_OOP_IN_OBJECT_EXCEPT_CLASS_RECORDING_WEAK_ROOTS(object, oopp) {
        if(oopp->is_mem()){
          mark_stack_.push(oopp->as_object());
        }
      }
...
Oop* GC_Thread_Class::lookUpNewLocation(Oop p){
  Oop p_unprotected = Oop::from_bits(p.bits() + The_Memory_System()->unprotected_heap_offset);
  Object* o = p_unprotected.as_object();
  if(o->is_forwardingPointer()){
...
void GC_Thread_Class::finalizeReference(Object_p weak_obj) {
  ...
  FOR_EACH_WEAK_OOP_IN_OBJECT(weak_obj, oop_ptr) {
    ...
    if (    x.is_mem()
      ...
      *oop_ptr = m_nilObj; // no store checks, no coherence operations, in the midst of GC
...
void GC_Thread_Class::setNilObj(Oop nilObj ){
  m_nilObj = nilObj;
}
...
bool GC_Thread_Class::isAlmostDead(Page* page){
