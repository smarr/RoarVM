...
/*
 ...
 class-Oop (at -4bytes) at bit number 12, instead of in the class-oop itself, since
...
public:
  ...
  static const int NMT_BIT_IN_BASE_HEADER_SHIFT = FormatShift + FormatWidth;
...
public:
  ...
  Oop* last_strong_pointer_addr_remembering_weak_roots();
...
# define FOR_EACH_STRONG_OOP_IN_OBJECT_EXCEPT_CLASS_RECORDING_WEAK_ROOTS(obj, oop_ptr) \
for (Oop* oop_ptr = (obj)->last_strong_pointer_addr_remembering_weak_roots();  oop_ptr > (obj)->as_oop_p();  --oop_ptr)
