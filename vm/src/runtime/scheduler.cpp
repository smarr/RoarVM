...
void Scheduler::switch_to_own_mutex() {
  ...
  if (shared_mutex == mtx) {
    ...
    assert(OS_Interface::mutex_trylock(newMutex));
    assert(OS_Interface::mutex_unlock(newMutex));
...
void Scheduler::transform_to_global_scheduler() {
  ...
  processorSchedulerPointer_obj()
                            ->storePointer(Object_Indices::ProcessListsIndex,
                                           result->as_oop());
...
void Scheduler::add_process_to_scheduler_list(Object_p process){
  assert(process->fetchPointer(Object_Indices::NextLinkIndex) ==
         get_interpreter()->roots.nilObj);
...
Oop Scheduler::find_non_running_process_for_core_between__ACQ(int hi, int lo, Squeak_Interpreter* runner) {
  ...
  FOR_READY_PROCESS_LIST_BETWEEN (hi, lo, slo, p, processList, get_interpreter())  {  
    ...
    for (;;) {
      assert(proc_obj->fetchClass() == 
             get_interpreter()->splObj(Special_Indices::ClassProcess));
      if (verbose) {
      ...
      bool running = proc_obj->is_process_running();
...
/* This function is used after the snapshot primitive, in which we need
 * to transfer to a specific method and ensure that it is removed from the
 * list. Some duplicated code from the transferToHighestPriority function
 * but don't see an immediate way to refactor it */

void Scheduler::remove_process_from_list(Oop newProc, const char* why) {
  Object_p process = newProc.as_object();
  Oop processListOop = process->my_list_of_process();
  assert(processListOop != get_interpreter()->roots.nilObj);
  Object_p processList = processListOop.as_object();
  ...
  if (first_proc == newProc) {
    processList->removeFirstLinkOfList();
    process->storePointer(Object_Indices::MyListIndex,
                          processListOop);
    return;
  }
  else if (last_proc == proc_oop) {
  ...
  for (;;) {
    assert(proc_oop != get_interpreter()->roots.nilObj);
    if (proc_oop == newProc) {
      processList->removeMiddleLinkOfList(prior_proc_obj, proc_obj);
      process->storePointer(Object_Indices::MyListIndex,
                            processListOop);
...
void Scheduler::transferTo(Oop newProc, const char* why){
  ...
  // Scheduler_Mutex sm("transferTo", get_interpreter()); // in case another core starts running this
...
void Scheduler::fixBackPointerOfProcess(Object_p process) {
  Object_p list = process->process_list_for_priority_of_process();
  process->storePointer(Object_Indices::MyListIndex,
                        list->as_oop());
