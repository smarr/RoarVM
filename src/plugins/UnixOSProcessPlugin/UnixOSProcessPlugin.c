...
dupToStdErr(sqInt anSQFileDataStructure) {
	...
	if (!((((interpreterProxy->isBytes(anSQFileDataStructure))
 && ((interpreterProxy->byteSizeOf(anSQFileDataStructure)) == (fileRecordSize())))
 && ((interpreterProxy->getThisSessionID()) == (sessionIdentifierFromSqFile(interpreterProxy->arrayValueOf(anSQFileDataStructure)))))
	...
	if ((sandboxSecurity()) == 1) {
		...
		if (((pid = vfork())) == 0) {
			...
			if (envVecBuffer == (interpreterProxy->nilObject())) {
				...
				idx1 = 0;
				while (idx1 < envCount) {
					ptr[idx1] = (envPtr + (((envOffsetPtr[idx1]) >> 1)));
					idx1 += 1;
				}
				env = ((char **) envPtr);
			}
			...
			idx2 = 0;
			while (idx2 < argCount) {
				ptr1[idx2] = (argsPtr + (((argOffsetPtr[idx2]) >> 1)));
				idx2 += 1;
			}
...
getStdHandle(sqInt n) {
	...
	memcpy(interpreterProxy->firstIndexableField(fileOop), &fileRecords[n], fileRecordSize());
	return interpreterProxy->popthenPush(1, fileOop);
}
...
primitiveAioDisable(void) {
	...
	/* begin fileDescriptorFrom: */
	if (!((((interpreterProxy->isBytes(sqFile))
 && ((interpreterProxy->byteSizeOf(sqFile)) == (fileRecordSize())))
 && ((interpreterProxy->getThisSessionID()) == (sessionIdentifierFromSqFile(interpreterProxy->arrayValueOf(sqFile)))))
		 && (isNonNullSQFile(sqFile)))) {
		fd = ((sqInt) -1);
		goto l1;
	}
	fd = ((sqInt) (fileno(fileHandleFrom(sqFile))));
l1:	/* end fileDescriptorFrom: */;
	if (fd < 0) {
...
primitiveAioEnable(void) {
	...
	if (!((interpreterProxy->isBytes(sqFile))
		 && ((interpreterProxy->stSizeOf(sqFile)) == (fileRecordSize())))) {
		return interpreterProxy->primitiveFail();
	}
	/* begin fileDescriptorFrom: */
	if (!((((interpreterProxy->isBytes(sqFile))
...
/*	Handle asynchronous event notification for a descriptor. The first
	parameter is
	a sqFile ByteArray representing the ioHandle for a file, and the remaining
	three parameters are Boolean flags representing the types of events for
	which notification
	is being requested: handle exceptions, handle for read, and handle for
	write. Flags are defined in the aio.h source as:
...
primitiveArgumentAt(void) {
	...
	if ((index > argCnt) || (index < 1)) {
		...
		strncpy(interpreterProxy->arrayValueOf(newString), sPtr, len);
		s = newString;
		interpreterProxy->pop(2);
...
primitiveGetCurrentWorkingDirectory(void) {
 ...
 && (bufferSize < maxSize))) break;
	...
	if (cwd == 0) {
		...
		len = strlen(cwd);
		newString = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classString(), len);
		strncpy(interpreterProxy->arrayValueOf(newString), cwd, len);
		cwdString = newString;
		interpreterProxy->pop(1);
...
primitiveRealpath(void) {
	...
	if (realpathResult == 0) {
		...
		newString = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classString(), len);
		strncpy(interpreterProxy->arrayValueOf(newString), realpathResult, len);
		s = newString;
		interpreterProxy->pop(2);
		interpreterProxy->push(s);
	}
...
/*	An array of Smalltalk Semaphore indices, one for each signal type for
	which a handler has been set. If an entry is zero, then no handler is
	active. 
 */
